...
⍝ NB: if you previously did   ApplySettings 'foo' followed by InitBrowser'', you should now combine these calls with InitBrowser 'foo'
    ...
    HOME←''    ⍝ the home folder of this ns in the filesystem
    ...
    ∇ home←_HOME;src
      ...
      :Else
          ...
          :AndIf 0=≢src←{0::'' ⋄ ⍵.SALT_Data.SourceFile}⎕THIS
              'Unable to determine home-directory and HOME is emppty!'⎕SIGNAL 11
          :EndIf
          HOME←1⊃⎕NPARTS src
      :EndIf
    ∇

    ∇ InitBrowser settings;browser;files;msg;path;len;options;opt;pth;subF;suffix;drv;opts;p;cap;BSVC;z;pckgs;Selenium;slnm;av;f;nm;f1;nugetPackage
      options←''
      :If 0=##.⎕NC'NugetConsum'  ⍝ this may happen if user just loaed Selenium.dyalog
          0 ## ⎕SE.SALT.Load _HOME,'NugetConsum.dyalog'
      :EndIf
